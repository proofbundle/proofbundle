...
Class Operator (O : Type) := {
  ...
  Budgeted : forall o x x',
    apply o x = Some x' ->
    Z.abs (coh_budget x' - coh_budget x) <= 1; (* ε = 1 for simplicity *)
  Identity : forall o x x',
    apply o x = Some x' ->
    root_id x' = root_id x
}.

(* Helper lemma for budget composition *)
...
(* Operator composition - Instance admitted for now, full proof can be developed separately *)
...
(* The `Extraction` command produces OCaml code that can be linked into the Rust VM (see the Makefile in the same directory). We only expose the two lemmas that the runtime needs to call: `fold_ctx_deterministic` and `event_horizon_sufficient_for_recovery`. *)
